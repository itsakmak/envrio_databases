...
from databases_library.engine import db_engine, engine, config
...
class User:
    ...
    @staticmethod
    def add(user: schemas.UsersTableCreate, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
        new_user = models.Users(name=user.name, email=user.email, subscription_expires_in=user.subscription_expires_in)
        db.add(new_user)
        db.commit()

    @staticmethod
    def get_by_name(name: str, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
        return db.query(models.Users).filter_by(name=name).first()
    
    @staticmethod
    def get_by_id(id: int, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
        return db.query(models.Users).filter_by(id=id).first()
    
    @staticmethod
    def get_by_email(email: str, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
        return db.query(models.Users).filter_by(email=email).first()
    
class Stations:

    @staticmethod
    def add(station: schemas.StationsCreate, db: Session=sessionmaker(bind=db_engine(engine, config),expire_on_commit=True)()):
    ...
    @staticmethod
    def get_by_code(code: str, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
        return db.query(models.Stations).filter_by(code = code).first()
    
    @staticmethod
    def get_by_brand(brand: str, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
        return db.query(models.Stations).filter_by(brand = brand).all()
    
    @staticmethod
    def get_by_access(user_id: int, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
    ...
    @staticmethod
    def update_date_created(station_id: int, new_entry: str, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
    ...
    @staticmethod
    def delete_by_code(code: str, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
...
class Gateways:
    ...
    @staticmethod
    def add(gateway: schemas.GatewaysCreate, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
...
class RemoteTerminalUnits:
    ...
    @staticmethod
    def add(rtu: schemas.RemoteTerminalUnitsCreate, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
    ...
    @staticmethod
    def get_by_station_id(station_id: int, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
        return db.query(models.RemoteTerminalUnits).filter_by(station_id=station_id).first()
    
class SensorsMeters:

    @staticmethod
    def add(sensor_meter: schemas.SensorsMetersCreate, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
    ...
    @staticmethod
    def get_by_rtu_id(rtu_id: int, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
        return db.query(models.SensorsMeters).filter_by(rtu_id=rtu_id).all()
    
    @staticmethod
    def get_by_station_id_and_rtu_id(station_id: int, rtu_id: int,db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
...
class MeasurementsTranslations:
    ...
    @staticmethod
    def add(translation: schemas.MeasurementTranslationsCreate, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
    ...
    @staticmethod
    def get_translation_by_measurement(measurement: str, db: Session=sessionmaker(bind=db_engine(engine, config), expire_on_commit=True)()):
